	...
	protected void login(Account account) {
		...
		if (account.needsRefresh() && !account.isOffline()) {
			...
			try {
				...
				DolphinClientCommon.getInstance().getLogger().info("Successfully logged in as " + current.getName());
				API.getInstance().startup(account);
			} catch (Exception e) {
				DolphinClientCommon.getInstance().getLogger().error("Failed to log in! ", e);
